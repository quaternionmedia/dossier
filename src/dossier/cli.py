...
def governance_load(corpus_dir: Optional[Path]) -> None:
    ...
    if not report.anything_loaded:
        click.echo(
            ...
            "If the corpus is vendored but the documents are absent, they are "
            "generated by ci/ in the corpus and reach a project when its\n"
            "governance pin is bumped past the commit that adds them."
        )
        raise SystemExit(1)
