...
def test_the_page_is_the_same_whatever_ran_before_it():
    """THE SCAFFOLDING WAS PART OF THE MEASUREMENT.

    `walkthrough/02-filling-it.md` registers `@cli.command("demo-limit")` on
    the real group to show that a rate limit is caught once, on the group. It
    is the real group, so the registration lasted for the rest of the process
    -- and this page, generated by a test in the same run, recorded
    `dossier demo-limit` as a command this application has, whenever the
    walkthrough happened to run first.

    Seventy-three commands on one ordering and seventy-two on another, from a
    documentation example. The page is committed, so the difference showed up
    as drift nobody could explain.

    Mutation: drop the `cli.commands.pop` line from the walkthrough and this
    fails.
    """
    from dossier.cli import cli

    left_behind = [name for name in cli.list_commands(None)
                   if name.startswith("demo-")]
    assert not left_behind, (
        f"something registered a command and did not remove it: {left_behind}")
