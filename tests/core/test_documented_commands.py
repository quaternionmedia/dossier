...
# **AN IMAGE EMBED IS NOT A LINK, AND CONFLATING THEM BROKE THIS ON CI.**
# `![alt](screenshots/a-sweep.gif)` names a picture the docs build records; it
# does not exist in a fresh checkout, and this guard called it a broken link.
# Pictures are `test_published_screenshots.py`'s subject and it already checks
# both directions -- committed or generated by a narrative, and nothing
# orphaned. The negative lookbehind is what keeps the two from overlapping.
LINK = re.compile(r"(?<!!)\[[^\]]*\]\(([^)#\s]+)(?:#[^)]*)?\)")
